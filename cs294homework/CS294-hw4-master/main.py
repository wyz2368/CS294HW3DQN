...
class NnValueFunction(object):
    ...
    def predict(self, X):
        if self.regressor == None:
            ...
            return np.array(list(self.regressor.predict_scores(X)))
...
def run(case):
    ...
    if case == 2:
        main_pendulum(logdir='./log/temp-test-pendulum-nn', gamma=0.97, animate=False, min_timesteps_per_batch=2500, initial_stepsize=1e-3, vf_type = 'nn', seed=0, desired_kl=2e-3)
    if case == 1 or case < 0:
